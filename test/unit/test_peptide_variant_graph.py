...
class TestPeptideVariantGraph(unittest.TestCase):
    ...
    def test_merge_join_heavy_node(self):
        r""" This test case simulates it when there are a lot of variants in
        ...
        graph, nodes = create_pgraph(data, 'ENST0001')
        graph.max_variants_per_node = 5
        graph.rule = 'trypsin'
        graph.merge_join(nodes[9])
        for node in list(graph.root.out_nodes)[0].out_nodes:
            self.assertTrue(len(node.variants) <= graph.max_variants_per_node)
            self.assertTrue(len(node.out_nodes) > 0)
    ...
    def test_call_and_stage_known_orf_cds_stop_gain(self):
        ...
        pool = VariantPeptideDict(graph.id)
        traversal = PVGTraversal(True, False, 0, pool, (0,90), (0,30))
